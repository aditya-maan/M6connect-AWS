Drupal.behaviors.custom_cropit = {
  attach: function (context, settings) {
    ...
    jQuery('.logo_old_remove').unbind('click').bind('click', function(event) {
      ...
      var nid_class = classes_split[4];
      var nid_split = nid_class.split('_');
      var nid = nid_split[1];
      jQuery.ajax({
        url: '/remove-img-from-node-ajax',
        type: 'POST',
        data: {nid: nid},
        beforeSend: function() {
          // setting a timeout
    ...
    jQuery('.banner_old_remove').unbind('click').bind('click', function(event) {
      ...
      var nid = nid_split[1];
      jQuery.ajax({
        url: '/remove-banner-from-node-ajax',
        type: 'POST',
        data: {nid: nid},
        beforeSend: function() {
          // setting a timeout
    ...
    jQuery(document).ajaxSend(function (event, XMLHttpRequest, ajaxOptions) {
      ...
      if ((urlajax.indexOf("/update-company-logo-photo") === 0 || urlajax.indexOf("/update-company-banner-photo") === 0) && ajaxOptions.hasOwnProperty("extraData")){
        jQuery.blockUI({
          baseZ: 2000,
