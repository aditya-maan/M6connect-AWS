...
if($node->nid && is_numeric($node->nid)){
$ProposalCount = get_rfp_proposals_all_content_count($node->nid);
...
<body>
 ...
 <!---- start content here in loop data ---->     
<?php 

    
       $m= 0; 
	   foreach($received_proposals as $delta => $receivedout) { 
           ...
           if($delta!=0){
			 print '<pagebreak suppress="off" />';
		  	...
		  	$proposal_number = $proposalNode->field_proposal_number['und'][0]['value'];
			$scope_of_work = $proposalNode->field_proposal_scope_of_work['und'][0]['value'];
			$Proposal_bid_amount = $proposalNode->field_proposal_bid_amount['und'][0]['value'];
			$Proposoal_bid_types = $proposalNode->field_proposoal_bid_types['und'][0]['value'];
			$proposal_submitted_for = $proposalNode->field_choose_from_the_options_li['und'][0]['value'];

			$propos_add_althoroughfare = $proposalNode->field_proposal_job_location['und'][0]['thoroughfare'];
			$propos_add_locality = $proposalNode->field_proposal_job_location['und'][0]['locality'];
			$propos_add_adminis = $proposalNode->field_proposal_job_location['und'][0]['administrative_area'];
			$propos_add_postalcode = $proposalNode->field_proposal_job_location['und'][0]['postal_code'];
	...
	'Submit Proposal to a company'=> array('field'=>'field_submit_proposal_to_company','type'=>'organization'),
		...
		if(array_key_exists($proposal_submitted_for,$fieldmapping)){
 		 if(isset($proposalNode->{$fieldmapping[$proposal_submitted_for]['field']}['und']) && !empty($proposalNode->{$fieldmapping[$proposal_submitted_for]['field']}['und'][0]['target_id'])){
    $submitedForContentNid = $proposalNode->{$fieldmapping[$proposal_submitted_for]['field']}['und'][0]['target_id'];
    ...
    //////////////// End Customer  Information ///////////////
		...
		$CompanyNid1  = _get_company_nid_by_group_content($proposalNode);
		$node_by_com = node_load($CompanyNid1);
    ...
    <?php
		 ...
		 /*if(!empty($node_by_com->field_logo['und'][0]['uri'])){	      
			$companyLogo = (isset($node_by_com->field_logo['und'][0]['uri'])) ? theme('image_style', array('style_name' => 'thumbnail', 'path' => $node_by_com->field_logo['und'][0]['uri'], 'getsize' => TRUE)) : '';  
			}else{
    		...
    		if (is_array($result) && !empty($result)) {
     			 foreach ($result as $delta => $fileObj) {
       			 ...
       			 if ($objFile) { 
		  			...
		  			$objFile->status = 0;
		  			file_save($objFile);
					$companyLogo= '<img alt="logo" src="'.$cimagepath.'"/>';  
        			}
      			}
    		}
    ...
    <table>
      <tr>
        ...
        <td style="width:450px; vertical-align:top;"><h1 style="text-align:center">
            <?php if(!empty($proposalNode->title)){ echo $proposalNode->title; }?>
          </h1>
         </td>
      </tr>
         <tr>
		    <td colspan="2" style="width:230px; vertical-align:bottom;"><h2 style="margin:0;"><?php echo $node_by_com->title; ?></h2></td>
         </tr>
    </table>
  </div>
  <div class="clearfix" style="margin:0 0 20px 0;">
    <div class="left_m6_view">
      ...
      <div class="m6_top_drywall clearfix">
        <h6><?php print $proposalNode->title; ?></h6>
      </div>
			...
			?>
                ...
                <li class="clearfix">
                 ...
                 <li class="clearfix">
                  ...
                  <span style="word-break:break-all; font-size:12px; word-wrap:break-word;"><?php echo $submittedUserInfo->mail; ?></span>
                  </li>
          ...
          <div class="m6_own" style="margin-right:0; padding-right:0;">
            ...
            <div class="m6_border">
              <ul class="m6_own_info clearfix">
                ...
                <li class="clearfix">
                  <label><strong>Name:</strong></label>
                  <span> <?php echo $node_by_com->title; ?> </span>
                </li>
                <li class="clearfix">
                  <label><strong>Address:</strong></label>
                  <span>
                ...
                <li class="clearfix">
                  <label><strong>City, State Zip:</strong></label>
                  <span><?php echo $Comlocality.' '.$Comadministrative_area.' '.$Compostal_code; ?></span>
                </li>
                <li class="clearfix">
                  <label><strong>Phone:</strong></label>
                  <span><?php echo $Comuserinfo->field_phone['und'][0]['value']; ?></span>
                </li>
                <li class="clearfix">
                  <label><strong>Email:</strong></label>
                  <span style="word-break:break-all; font-size:12px; word-wrap:break-word;"><?php echo $Comuserinfo->mail; ?></span>
                </li>
              </ul>
            </div>
          </div>
        ...
        <div id="page-wrap1_viewprop">
          <table cellpadding="0" cellspacing="0" border="0">
            <tbody>
              ...
              <?php
					$time = $proposalNode->field_proposal_document_picture['und'];
