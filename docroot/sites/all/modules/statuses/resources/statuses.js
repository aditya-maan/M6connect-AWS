    ...
    Drupal.behaviors.statuses = {
        attach: function (context) {
            ...
            ctxt.find('#statuses-box').bind('ajax_complete', function (context) {
                ...
                fbss_refresh();
               /// refresh_dashboard_company(); //18-7-2016 dashboard image upload

            });
            ctxt.find('[id^=statuses-box]').bind('ajax_complete', function (context) {
...
//            ctxt.find('#statuses-box--3').bind('ajax_complete', function (context) {
//
//                if ($(context.target).html() != $(this).html()) {
//                    return;
//                }
//                fbss_refresh();
//                refresh_dashboard_company
//            });
//            ctxt.find('#statuses-box--4').bind('ajax_complete', function (context) {
//
//                if ($(context.target).html() != $(this).html()) {
//                    return;
//                }
...
//                refresh_dashboard_company
//            });
//            ctxt.find('#statuses-box--5').bind('ajax_complete', function (context) {
//
//                if ($(context.target).html() != $(this).html()) {
//                    return;
//                }
...
/*    function refresh_dashboard_company() {
     var location = window.location.pathname +'?';
    // Build the relative URL with query parameters.
    var query = window.location.search.substring(1);
    if ($.trim(query) != "") {
    ...
    jQuery.ajax({
        url: location +"page=dashboard&ts="+ (new Date()).getTime(),
        context: document.body
    }).done(function (data) {
        var new_data = data.replace(/<script(.|\s)*?\/script>/g, "");
        var new_content = jQuery('<div></div>').html(new_data);
        if (new_content) {
             
            var element = jQuery('#cutom_statuses_stream_div');
            var insert = new_content.find('#cutom_statuses_stream_div');
            if (insert.length && insert.length > 0 && element.length && element.length >= insert.length) {
...
}*/  //18-7-2016 dashboard image upload
})(jQuery);
