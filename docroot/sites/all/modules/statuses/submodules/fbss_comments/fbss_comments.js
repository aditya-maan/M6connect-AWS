    ...
    Drupal.behaviors.fbss_comments = {
        attach: function (context) {
            ...
            ctxt.find('.fbss-comments-show-comment-form').one('click', function () {
                $(this).hide();
//        var f = $('#' + this.id + ' + div');
                var f = $(this).parent().parent().parent().next();
                f.fadeIn(300);
                var sid = this.id.split('-').pop();
//        f.find('.fbss-comments-replace-' + sid + '-inner').fadeIn(300);
//        $('#statuses-item-' + sid + ' .fbss-comments-hide').fadeIn(300);
//        $('#statuses-item-' + sid + ' .fbss-comments-hide').find('.fbss-comments-textarea').focus();
                f.find('.fbss-comments-replace-' + sid + '-inner').fadeIn(300);
                f.find('.fbss-comments-hide').fadeIn(300);
                f.find('.fbss-comments-hide').find('.fbss-comments-textarea').focus();
            ...
            ctxt.find('.fbss-comments-comment-form').bind('ahah_success', function () {
                $(this).parents('form').find('.fbss-comments-submit').attr('disabled', true);
            });
