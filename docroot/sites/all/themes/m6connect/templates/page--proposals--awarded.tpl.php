...
?>
                          <div class="clearfix margin-bottom-10">
                                      <form name="listSearcFrm" id="seach_award" method="GET" >
                                        <div class="custom-projects-lists-search pull-left">
                                         <input placeholder="Search Proposals" name="search_proposal" value="<?php print $defult_search; ?>" type="text" class="form-text form-control">
                     ...
                     <input type="submit" class="search_m6_prop1 form-submit" name="search" id="search" value="Search"/>
                                         ...
                                         <div class="custom-projects-lists-sorting pull-right">
                                          <select style="display:none;" class="form-select" name="sel_name" onchange="if (this.value) window.location.href='?sel_name='+this.value">
	...
	//$prop_postfirstname = $node_owner_obj->field_first_name['und'][0]['value'];
	 ...
	 /*     * ************ end brijendra Work ************* */
        ...
        if($targetNodetype=='organization'){
				...
				/*if(((($userid1 == $logged_user) && ($CurrCompNid == _get_company_nid_by_group_content($targetNode))) || (($node_owner == $logged_user) &&($CurrCompNid == _get_company_nid_by_group_content($propNode)))) && ($node_pro_id == $node_award_id)|| ($company_admin_rols_access == TRUE)) {*/
				
			$proposalSubmitterAccess = ((($node_award_id == $node_pro_id) && ($CurrCompNid == _get_company_nid_by_group_content($propNode))) && ((($node_owner == $logged_user))))?1:0;
$proposalReceiverAccess = ((($node_award_id == $node_pro_id) && (in_array($CurrCompNid,_get_company_nids_by_proposal_target_node($targetNode)))) && ((($userid1 == $logged_user) || (is_proposal_project_collabarated($propNode)))))?1:0;
...
?>
	...
	//logo image ends
				...
				if(isset($_SESSION['notification_user'])){
				  ...
				  if(isset($notification['notification_proposal']['count']) && $notification['notification_proposal']['count']> 0 && isset($notification['notification_proposal']['awarded']) 
					&& isset($notification['notification_proposal']['awarded']['entity_id']) && !empty($notification['notification_proposal']['awarded']['entity_id'])){
					if(in_array($node_pro_id, $notification['notification_proposal']['awarded']['entity_id'])){
			    ...
			    ?>
                                      <li class="proposal-m6-container prop_m6con1<?php print $notificationClass; ?><?php print $isUpgradeRequiredClass;?>">
                                            ...
                                            <div class="btn-group" style="margin-top:40px;">
                                              <div class="dropdown">
                                                ...
                                                <ul aria-labelledby="dLabel" class="dropdown-menu" role="menu">
                                                  ...
                                                  <?php if($proposalSubmitterAccess) { ?>
...
<?php
  ...
  $query_3 = db_select('users_roles', 'ur') 
  ->fields('ur', array('uid', 'rid')) ;
  $db_or1 = db_or();  
  $db_or1->condition(db_and()->condition('ur.uid', $user->uid, '!=')->condition('ur.rid', array(1, 2, 3, 5, 6), 'NOT IN'));
  $src_2 =  $query_3->condition($db_or1);  
  $result_3 = $src_2->execute();  
  foreach($result_3 as $item_3)
  { 
    $user_order_role1 = $item_3->rid; 	 
  } 
...
jQuery('#paid_membership_dialog_accept_award','.page-proposals-awarded').dialog({
  autoOpen: false,
  width: 550,
  modal: true,
  resizable: false,
...
jQuery('#requires_membership_upgrade_dialog_accept_award','.page-proposals-awarded').dialog({
  autoOpen: false,
  width: 550,
  modal: true,
  resizable: false,
...
var my_role1 = "<?php echo $user_order_role1; ?>";
