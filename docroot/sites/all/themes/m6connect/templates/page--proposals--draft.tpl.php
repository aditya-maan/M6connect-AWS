...
if (empty(arg(2))) {
  ...
  WHERE a.status=0 AND cm.field_cm_type_value IS NULL AND a.type=:type '.$title_search, array(':type'=>'my_proposal',':ReplaceFrom'=>',',':ReplaceTo'=>'',':MPEntityType'=>'node'));
}
else {
  $sql_node3 = db_query('SELECT a.title,a.nid,a.uid,a.status,a.created,b.entity_id,b.field_proposal_number_value,
  c.entity_id,c.field_submit_proposal_in_respons_target_id,
  ...
  LEFT JOIN proposal_award AS j ON a.nid=j.node_id  
  LEFT JOIN proposal_reject AS k ON a.nid=k.reject_node  
  WHERE a.status=0 AND cm.field_cm_type_value = :cm_type AND a.type=:type '.$title_search, array(':type'=>'my_proposal',':ReplaceFrom'=>',',':ReplaceTo'=>'',':MPEntityType'=>'node', ':cm_type' => arg(1)));
}
...
if($sql_node4 && !empty($sql_node4)){
  $sql_node	= json_decode(json_encode($sql_node4), true);
	 ...
	 /*     * ************ end brijendra Work ************* */
				...
				/*if((($node_owner == $logged_user) && ($node_reject_id != $node_pro_id) &&($CurrCompNid == _get_company_nid_by_group_content($propNode))) || (($node_owner == $logged_user) && ($node_award_id != $node_pro_id) &&($CurrCompNid == _get_company_nid_by_group_content($propNode))) || ($company_admin_rols_access == TRUE)){*/	
				if((($node_reject_id!=$node_pro_id) && ($node_award_id != $node_pro_id) && ($CurrCompNid == _get_company_nid_by_group_content($propNode))) && ((($node_owner == $logged_user) || $company_admin_rols_access))){
	...
	/*		    if($propoalCmpNode && isset($propoalCmpNode->field_logo) && isset($propoalCmpNode->field_logo['und']) && $propoalCmpNode->field_logo['und'][0]['fid']){
		       ...
		       ?>
                                      <li class="prop_m6con1 proposal-m6-container">
                                            ...
                                            <div class="btn-group" style="margin-top:40px;">
                                              <div class="dropdown">
                                                ...
                                                <ul aria-labelledby="dLabel" class="dropdown-menu" role="menu">
                                                  ...
                                                  <li><a href="/<?php echo 'node/'.$node_pro_id.'/edit';?>">Edit</a></li>
                                                </ul>
                                              </div>
                                            </div>
                                          </div>
