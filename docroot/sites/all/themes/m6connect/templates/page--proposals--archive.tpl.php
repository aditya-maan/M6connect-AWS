...
if (empty(arg(2))) {
  ...
  WHERE pa.field_is_proposal_archived_value=1 AND cm.field_cm_type_value IS NULL AND a.status=1 AND a.type=:type '.$title_search, array(':type'=>'my_proposal',':ReplaceFrom'=>',',':ReplaceTo'=>'',':MPEntityType'=>'node',':AccessPurpose'=>'amount_access',':AssessTimestamp'=> strtotime('-20 minutes'),':ProposalAmount'=>$sortingProposalAmount));
}
else {
  $sql_node3 = db_query('SELECT a.title,a.nid,a.uid,a.status,a.created,b.entity_id,b.field_proposal_number_value,
  c.entity_id,c.field_submit_proposal_in_respons_target_id,
  ...
  LEFT JOIN proposal_award AS j ON a.nid=j.node_id  
  LEFT JOIN proposal_reject AS k ON a.nid=k.reject_node  
  WHERE pa.field_is_proposal_archived_value=1 AND cm.field_cm_type_value = :cm_type AND a.status=1 AND a.type=:type '.$title_search, array(':type'=>'my_proposal',':ReplaceFrom'=>',',':ReplaceTo'=>'',':MPEntityType'=>'node',':AccessPurpose'=>'amount_access',':AssessTimestamp'=> strtotime('-20 minutes'),':ProposalAmount'=>$sortingProposalAmount, ':cm_type' => arg(1)));
}
