...
<div class="m6_view_proposal">
  <div class="m6_main_view">
    ...
    <ul id="content_msg" class="proposal-sent-scroll">
<?php
...
/*$sql_node1 = db_query('SELECT DISTINCT a.nid,a.title,a.uid,a.status,a.created,b.entity_id,b.field_proposal_number_value,
c.entity_id,c.field_submit_proposal_in_respons_target_id,
d.entity_id,d.field_field_sub_proposal_pjt_target_id,
e.entity_id,e.field_submit_proposal_member_target_id,
f.entity_id,f.field_submit_proposal_to_company_target_id,
...
WHERE (a.status=1 AND a.type=:type) '.$title_search.' LIMIT 1',array(':type'=> 'my_proposal'));
$sql_node1_ret = $sql_node1->fetchAll();*/


$sql_node1 = db_select('node','a');
$sql_node1->leftJoin('field_revision_field_proposal_number','b','a.nid=b.entity_id');
$sql_node1->leftJoin('field_data_field_submit_proposal_in_respons','c','a.nid=c.entity_id');
$sql_node1->leftJoin('field_revision_field_field_sub_proposal_pjt','d','a.nid=d.entity_id');
$sql_node1->leftJoin('field_revision_field_submit_proposal_member','e','a.nid=e.entity_id');
$sql_node1->leftJoin('field_revision_field_submit_proposal_to_company','f','a.nid=f.entity_id');
$sql_node1->leftJoin('field_data_field_proposal_bid_amount','g','a.nid=g.entity_id');
$sql_node1->leftJoin('field_data_field_proposoal_bid_types','h','a.nid=h.entity_id');
$sql_node1->leftJoin('field_data_field_proposal_job_location','i','a.nid=i.entity_id');
$sql_node1->leftJoin('proposal_award','j','a.nid=j.node_id');
$sql_node1->leftJoin('proposal_reject','k','a.nid=k.reject_node');
$sql_node1->distinct();
$sql_node1->fields('a', array('nid','title','uid','created'));
$sql_node1->fields('b', array('entity_id','field_proposal_number_value'));
$sql_node1->fields('c', array('field_submit_proposal_in_respons_target_id'));
$sql_node1->fields('d', array('field_field_sub_proposal_pjt_target_id'));
$sql_node1->fields('e', array('field_submit_proposal_member_target_id'));
$sql_node1->fields('f', array('field_submit_proposal_to_company_target_id'));
$sql_node1->fields('g', array('field_proposal_bid_amount_value'));
$sql_node1->fields('h', array('field_proposoal_bid_types_value'));
$sql_node1->fields('i', array('field_proposal_job_location_administrative_area','field_proposal_job_location_locality', 'field_proposal_job_location_postal_code'));
$sql_node1->fields('j', array('node_id','status','award_taker_id','award_giver_id'));
$sql_node1->fields('k', array('reject_node','status','reject_taker_id','reject_giver_id'));
$sql_node1->condition('a.status',1,'=');
$sql_node1->condition('a.type','my_proposal','=');
$sql_node1->extend('PagerDefault');
$sql_node1->range(0,5);
$sql_node1_ret = $sql_node1->execute()->fetchAll();

//pre($sql_node1_ret,1);
$sql_node = array();
foreach($sql_node1_ret as $delta => $sqlObj){
  $sql_node[$delta] = (array)$sqlObj;
}
...
?>		  
	...
	<?php  	} } ?>			
		  ...
		  pager_default_initialize($total, 5, $element = 0);
$output = theme('pager');
print $output;
		  ?>
...
$(".search_m6_prop1").on("click", function () {
 ...
 /* if(jQuery('body').hasClass('page-proposals-sent')){
	 // alert(numItems);
	jQuery.ias({
	  container : '.proposal-sent-scroll', 
	  item: '.proposal-m6-container',
	  pagination: '#pagination',
	  next: 'li .next a',
